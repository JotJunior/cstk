...
- [x] 6.1.1 Reescrever `scenario_resume_agente_instrui_mcp_stop_terminal` (`tests/test_command-spawn-mcp-lifecycle.sh:121`) e o cenario irmao de `feature-00c` (`:125`) para o contrato novo: o PROCESSO do servidor e coextensivo com a sessao do harness (FR-012), mas a SESSAO MCP (descritor + token) continua sobrevivendo a pausas entre ondas — nao confundir os dois (data-model.md §State transitions). Evidencia: os 2 scenarios que precisavam de reescrita nao eram `scenario_resume_*_instrui_mcp_stop_terminal` (essas so checam a presenca de `cstk mcp stop --state-dir`, inalterado), e sim `scenario_resume_agente_nao_para_em_aguardando_humano`/`scenario_resume_feat_nao_para_em_aguardando_humano` (confirmado via `git show 3af0f75:tests/test_command-spawn-mcp-lifecycle.sh | grep -n '^scenario_'` — linhas 121/125 exatas na epoca do plan.md). O texto fonte em `plugins/cstk/commands/agente-00c-resume.md`/`feature-00c-resume.md` (bloco `> Estado aguardando_humano NAO e terminal`) afirmava `FR-010: a sessao do servidor e coextensiva com a execucao inteira — o servidor MUST permanecer ativo durante a pausa`, que MENTIA apos a FASE 5 (nao ha mais processo persistente). Reescrito para distinguir SESSAO MCP (descritor+token, sobrevive) de PROCESSO (coextensivo com a sessao do harness, FR-012). Os 2 scenarios de teste ganharam 2 asserts novos cada (`grep -Eiq 'SESSAO MCP'` + `grep -Eiq 'processo.*coextensivo.*sessao do harness'`), alem do assert original preservado. `LC_ALL=C ./tests/run.sh command-spawn-mcp-lifecycle` → `PASS 27 FAIL 0 ERROR 0 ORPHANS 0`
- [x] 6.1.2 Adicionar cobertura, no mesmo arquivo ou em `tests/test_mcp-launch.sh`/`tests/test_mcp-session.sh`, do fluxo de resolucao por chamada (cache hit + miss) no nivel de comando/orquestrador. Evidencia: gap real identificado — os testes pre-existentes de `resolveSessionForCall` hit/miss (resolve.test.ts:141-190) usavam fixtures fake (`fake-mcp-session-ok.sh`), nunca o `mcp-session.sh` REAL; o unico teste com helper real (task 1.2.7) exercitava so revalidacao pos-terminacao, nao a transicao miss->hit. Novo teste `resolveSessionForCall (task 6.1.2, mcp-session.sh REAL)` em `mcp/state-server/test/resolve.test.ts` monta um projeto tmpdir descartavel com layout real (`.claude/agente-00c-state/mcp-server.json`), resolve 1a chamada via tree-walk `--project-path` REAL (miss, popula cache), 2a chamada com projectPath deliberadamente invalido (hit, prova que usa `--state-dir` cacheado sem tree-walk de novo). `cd mcp/state-server && npm test` → `tests 127 / pass 127 / fail 0` (era 125; ver Decisao dec-082)

### 6.2 Cobertura de multi-sessao (FR-011) `[C]`

Ref: quickstart.md Cenario 7 passo 5; data-model.md §Relacionamentos

- [x] 6.2.1 Adicionar/estender teste cobrindo duas execucoes ativas simultaneas (`agente-00c` + `feature-00c`) com tokens distintos, confirmando que cada chamada muta exclusivamente o state-dir da sessao cujo token foi apresentado — nenhum cross-talk (mudanca de cardinalidade 1 processo : N sessoes). Evidencia: novo teste `resolveSessionForCall (task 6.2.1, FR-011)` em `mcp/state-server/test/resolve.test.ts` — 2 projetos tmpdir descartaveis (`mkdtemp`, NUNCA a execucao real desta feature — dec-075), um `agente-00c` e um `feature-00c`, tokens distintos, MESMO `SessionCache` compartilhado (simula 1 processo : N sessoes). Confirma miss->hit para os 2 tokens intercalados e que `sessionA2.stateDir !== sdB` / `sessionB2.stateDir !== sdA` (a resolucao e o mecanismo real que os handlers de tool usam para decidir qual state-dir mutar — record_decision/open_wave/etc. sempre operam sobre `session.stateDir` do objeto resolvido; nao ha caminho de mutacao que bypasse essa resolucao, logo cobrir a resolucao cobre a garantia de isolamento). `cd mcp/state-server && npm test` → `tests 127 / pass 127 / fail 0`
...
- [x] 6.5.1 `npm test` (dentro de `mcp/state-server/`) + `LC_ALL=C ./tests/run.sh --check-coverage` + `LC_ALL=C ./tests/run.sh mcp` + `LC_ALL=C ./tests/run.sh` (suite POSIX completa) todos verdes. Evidencia: `npm test` → `tests 127 / pass 127 / fail 0` (era 125; +2 dos tasks 6.1.2/6.2.1); `LC_ALL=C ./tests/run.sh --check-coverage` → "Cobertura completa: zero orfaos"; `LC_ALL=C ./tests/run.sh mcp` → `PASS 143 FAIL 0 ERROR 0 ORPHANS 0`; `LC_ALL=C ./tests/run.sh` (suite POSIX completa, foreground, ~17min) → `PASS 3005 FAIL 0 ERROR 0 ORPHANS 0 TIME 1018s`
