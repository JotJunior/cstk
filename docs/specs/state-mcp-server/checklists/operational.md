...
- [ ] CHK064 - Existe requisito para container remanescente quando o command pai termina SEM chamar `stop` (crash, `kill -9`, sessao encerrada)? [Gap, Spec §US2 vs contracts §Ciclo de vida] {auto} — US2 exige "sem processos orfaos sobrevivendo entre sessoes", mas o ciclo de vida so descreve o caminho feliz `start`/`stop`; nao ha requisito de deteccao/limpeza de container stale (paridade com o lock, que research.md registra como "**sem deteccao de stale**").
...
- [ ] CHK071 - Ha requisito definindo COMO o orquestrador detecta a indisponibilidade no meio da onda e comuta para o caminho Bash? [Gap, Spec §FR-007 vs §US4 cenario 2] {auto} — FR-007 e US4 cenario 1 cobrem a deteccao **antes** de delegar (feita pelo command pai); nenhum requisito define o gatilho, o numero de tentativas nem o ponto de comutacao quando a queda ocorre **apos** a primeira tool ter sido chamada — US4 cenario 2 exige apenas que o estado nao fique pior.
