...
- [ ] CHK009 - O comportamento do lote com o MESMO `--field` repetido (last-wins? erro de uso?) esta definido? [Gap, Contract §1] {auto} — nem spec FR-005 nem contract §1 definem duplicata de campo no lote; vira tarefa "definir semantica de campo duplicado no parser N pares" (create-tasks consome).
...
- [ ] CHK008 - As assinaturas marcadas `[PROPOSTA — a validar na implementacao]` (set multi-campo, semantica do --force, _state-read.sh) tem passo definido de validacao + remocao do marcador ao implementar? [Gap, Contract §1/§2/§4] {auto} — o contract marca as propostas mas nenhum artefato define QUANDO/QUEM valida e atualiza; vira tarefa "validar assinaturas PROPOSTA contra implementacao e atualizar contract" (create-tasks consome).
