...
| **VI. Snapshot que muda** | **PASS** | Sem conexao de longa duracao: `bridge.db` aberto por request e fechado no `finally` das rotas (`routes/bridge.ts`) **quando `openBridgeDb()` retorna com sucesso**, e fechado dentro do proprio `openBridgeDb()` (`db/bridge.ts`) **quando um passo pos-abertura falha** (pragmas/`quick_check`/DDL) — corrigido na convergencia (achado 6.4): antes, uma falha nesse trecho deixava a atribuicao do chamador incompleta e o `finally` virava no-op sobre `null`, vazando o handle. `freshness` do envelope da Ponte vem do `mtime` do `bridge.db` (`wrapBridge()`), nao do corpus. |
