...
| R4 | `title`/`description`/`default` nao renderizarem ⇒ texto de FR-002 nao chega ao operador | Media | Desenho **nao depende** dos tres; se falharem, texto migra para `message` (campo **obrigatorio** do schema, `types.d.ts:4983` — nao opcional como os tres). Marcado `[PROPOSTA]` no contrato. Desde dec-047 o **aviso de risco de rebaixamento** ja nasce em `message`; a renderizacao do proprio `message` e a unica premissa restante, medida no Scenario 0 |
