...
|-----|--------|-------------------------------|
| MEDIUM | **ReDoS no redactor interno** (CWE-1333). O redactor e regex sobre conteudo hostil de ate megabytes; o padrao de bloco `BEGIN ... PRIVATE KEY` e multi-linha e o candidato natural a backtracking catastrofico | **Criterio implementavel (nao mais "deve ser seguro")**: (1) todo padrao do redactor e **ancorado** (`^`/`$` por linha, nunca varredura livre no meio do texto) ou usa limite explicito de repeticao; (2) **zero quantificadores aninhados** no estilo `(a+)+`/`(a\|b)*c` sobre o mesmo grupo de caracteres — proibido por revisao de codigo, nao apenas por convencao; (3) o bloco `-----BEGIN ... PRIVATE KEY-----` e casado **linha-a-linha** por uma maquina de estados simples (flag booleana "dentro do bloco", set/clear em `BEGIN`/`END`), **nunca** por um unico regex guloso que atravessa o texto inteiro. Medicao empirica (nao suposta) do custo de cada abordagem: `quickstart.md` Scenario 12.1 |
