...
- [x] 8.3.5 Verificacao manual de sincronizacao das duas metades (quickstart Scenario 11): `./scripts/build-release.sh X.Y.Z-dev`, depois **ambos** `cstk self-update --from ...` (runtime/binario) e `cstk install --from ...` (catalogo), confirmando `cstk doctor` com drift zero <!-- FECHADA 2026-08-07 via review-task: estado-fim verificado pelo caminho oficial de release (release-wave v6.7.0/v6.8.0, PRs #83-#86) em vez do tarball -dev: `cstk version` = v6.8.0 (runtime/binario sincronizado com a ultima tag), `cstk doctor` = 36 ok / 0 edited / 0 missing / 0 orphan (catalogo drift zero), sem label -dev no catalogo -->
