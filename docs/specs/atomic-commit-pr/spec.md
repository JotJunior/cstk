...
- **FR-004**: When `atomic_commit_enabled = true`, the orchestrator MUST create one git commit per successfully completed task (outcome=pass) during `execute-task`. Tasks completed with outcome=pass in the same wave are ALWAYS grouped into a single ranged commit (grouping is always-on per wave — no separate flag required). A wave with a single passing task receives its own individual commit.
...
- **FR-007**: Commit messages MUST follow Conventional Commits format and include the pipeline stage name, feature/project name, and a brief description. In pipeline (non-interactive) mode, the orchestrator uses `git commit -m "<message>"` directly with a message generated by `commit-mode.sh stage-message` or `task-message`. The installed `commit` skill is NOT used in automated pipeline mode (it has interactive prompts incompatible with autonomous execution); it MAY be used when an operator manually triggers a commit outside the pipeline.
- **FR-008**: When `atomic_commit_enabled = true` and the pipeline reaches terminal success, the orchestrator MUST push the session branch to the remote and open a PR. If the current branch is a `cstk session` branch (worktree exists), use `cstk session pr <NAME>`; otherwise push via `git push -u origin HEAD` and open PR via `gh pr create`. The `NAME` parameter for `cstk session pr` is derived from: `short_name` (feature-00c) or `basename(target_project_path)` (agente-00c).
