...
- [ ] 1.1.1 Criar `global/skills/checklist/scripts/requirement-coverage.sh`
...
- [ ] 1.1.2 Implementar parsing de `FILE` + flag opcional `--min-match N`
      (default 2, inteiro >= 1; `--min-match` invalido ou `FILE` ausente →
      exit 2, contract §Exit codes)
- [ ] 1.1.3 Implementar extracao de IDs+enunciados das linhas
      `- **FR-NNN**:` sob `### Functional Requirements` (enunciado inclui
      linhas de continuacao ate o proximo item ou fim de secao — contract
      §Algoritmo passo 1)
- [ ] 1.1.4 Implementar montagem do corpus (Acceptance Scenarios de todas as
      User Stories + `### Edge Cases`, lowercase, pontuacao removida —
      contract §Algoritmo passo 2)
- [ ] 1.1.5 Implementar fast-path: corpus cita `FR-NNN` literal → coberto
      (contract §Algoritmo passo 3.a)
- [ ] 1.1.6 Implementar heuristica textual (spec FR-005): tokenizar
...
- [ ] 1.1.7 Emitir `FINDING|error|fr-no-scenario|FR-NNN sem cenario
      associado — ...` para cada gap (ID exato + fix acionavel — spec
      FR-003, SC-002)
- [ ] 1.1.8 Emitir `RESULT|<FILE>|requirements=<T>|covered=<C>|errors=<N>`
      sempre, inclusive spec sem FRs (`requirements=0|covered=0|errors=0`,
      exit 0 — spec FR-004)
- [ ] 1.1.9 Definir exit code final: 0 sem gaps, 1 com >= 1 gap, 2 uso
      incorreto (contract §Exit codes)
- [ ] 1.1.10 Escrever `tests/test_requirement-coverage.sh`: gap unico
      ...
      §Teste
- [ ] 1.1.11 Rodar `./tests/run.sh --check-coverage` e confirmar que
...
- [ ] 1.2.1 Adicionar chamada de `requirement-coverage.sh` na ETAPA 4
...
- [ ] 1.2.2 Adicionar a mesma chamada em `global/skills/checklist/SKILL.md`
      (ETAPA 4: SALVAMENTO), antes de reportar o checklist como concluido
- [ ] 1.2.3 Documentar em ambas as SKILL.md o registro de execucao autonoma
      (`state-ondas.sh record-skill --skill requirement-coverage --kind
      gate`, script deterministico — nao tool Skill)
- [ ] 1.2.4 Validar manualmente contra 2-3 specs reais do repo
...
- [ ] 2.1.1 Adicionar subsecao nova em `global/skills/specify/SKILL.md`
      ...
      feature
- [ ] 2.1.2 Documentar que a recomendacao MUST citar o criterio aplicado
      (ex.: "mesma intencao original, refinamento de escopo" vs "nova
      capacidade nao coberta pela spec X")
- [ ] 2.1.3 Documentar o caso trivial (FR-008): nenhuma spec existente se
      relaciona ao pedido → prosseguir direto para nova feature, sem
      overhead de decisao adicional
- [ ] 2.1.4 Validar a prosa lendo o SKILL.md renderizado (sem gate
...
- [ ] 2.2.1 Adicionar nota em `global/skills/clarify/SKILL.md` ETAPA 2
...
- [ ] 2.2.2 Validar consistencia textual entre a nota de clarify e a
...
- [ ] 3.1.1 Atualizar `global/skills/review-features/SKILL.md` (secao que
...
- [ ] 3.1.2 Adicionar nota explicita: diretorios ja existentes sob
...
- [ ] 3.1.3 Conferir que nenhum outro consumidor dinamico (scripts,
...
- [ ] 3.1.4 Validar a prosa lendo o SKILL.md renderizado (item textual,
...
- [ ] 4.1.1 Criar `global/skills/agente-00c-runtime/scripts/_diag.sh`
      sourceable (padrao `_log.sh`/`_hash.sh` — NAO executavel diretamente),
      expondo `diag_emit <severity> <code> <message> <fix>`
- [ ] 4.1.2 Validar `severity` em `{error, warning}` (best-effort: nao
      mascarar o erro original do script chamador se a validacao falhar)
- [ ] 4.1.3 Validar `fix` MUST NOT ser identico a `message` (spec FR-013)
- [ ] 4.1.4 Escapar `|` interno de `message`/`fix` substituindo por `/`
      antes de emitir
- [ ] 4.1.5 Emitir em stderr, 1 linha: `DIAG|<severity>|<code>|<message>|<fix>`
      (contract §Saida) — zero dependencia de `jq` (spec FR-016)
- [ ] 4.1.6 Escrever `tests/test__diag.sh`: emissao dos 4 campos corretos;
      ...
      → `test__hash.sh`)
- [ ] 4.1.7 Rodar `./tests/run.sh --check-coverage` e confirmar que
...
- [ ] 4.2.1 Ler os paths de erro reais de `state-rw.sh` e confirmar as 3
...
- [ ] 4.3.1 Ler os paths de erro reais de `state-lock.sh` e confirmar as 2
...
- [ ] 4.3.2 Adicionar `diag_emit` ADITIVO nos 2 pontos de falha, mensagem
      legada intacta
- [ ] 4.3.3 Estender `tests/test_state-lock.sh`: assercao de `DIAG|` nas 2
...
- [ ] 4.4.1 Ler os paths de erro reais de `state-ondas.sh` e confirmar as 2
...
- [ ] 4.4.2 Adicionar `diag_emit` ADITIVO nos 2 pontos de falha, mensagem
      legada intacta
- [ ] 4.4.3 Estender `tests/test_state-ondas.sh`: assercao de `DIAG|` nas
...
- [ ] 4.5.1 Ler os paths de erro reais de `bloqueios.sh` e confirmar a
...
- [ ] 4.5.2 Adicionar `diag_emit` ADITIVO no(s) ponto(s) de falha,
      mensagem legada intacta
- [ ] 4.5.3 Estender `tests/test_bloqueios.sh`: assercao de `DIAG|` na
...
- [ ] 4.6.1 Confirmar que nenhum script POSIX fora dos 4 do escopo-piloto
...
- [ ] 4.6.2 Rodar `./tests/run.sh` completo (suite inteira, nao so
      ...
      scripts-piloto e nos demais ~1100+ cenarios
