...
b2. ASSERT nao-symlink: state-dir, rounds/, staging, alvo     -> senao exit 1
...
d. escreve rounds/.rotate-journal (phase=staged)
e. mkdir rounds/.<label>.staging   (chmod 700 best-effort)
f. mv -- de cada arquivo transacional -> staging   (phase=moving)
...
`state-history/`, `backups/`, `enforcement-log.jsonl`, `commit-baseline.txt`,
...
| journal + staging incompleto | interrompida durante os `mv` | roll-back: devolve arquivos a raiz, remove staging + journal |

"Staging completo" = todos os nomes de `files` presentes em `staging`, por
`[ -f ]`. Sem dependencia externa.
...
| T-06 | apos `rotate` sqlite, `rounds/<l>/` contem **so** `state.db`; nenhum `-wal`/`-shm` | Decision 2, v6.4.0 |
