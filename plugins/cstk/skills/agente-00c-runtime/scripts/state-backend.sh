...
_sb_check_capability() {
  ...
  elif [ -n "${CSTK_LIB:-}" ] && [ -f "$CSTK_LIB/../../plugins/cstk/skills/agente-00c-runtime/scripts/state-backend.sh" ]; then
    _SB_CAP_ORIGIN="arvore-do-repo"
    _sbcc_dir=$(CDPATH= cd -- "$CSTK_LIB/../../plugins/cstk/skills/agente-00c-runtime/scripts" 2>/dev/null && pwd -P)
