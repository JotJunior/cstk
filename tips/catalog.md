...
category: uso
text: Use /create-use-case para documentar um caso de uso funcional com fluxos, atores, regras de negocio e casos de teste — formato UC-DOMINIO-NNN.
---
Para documentar um caso de uso complexo:

```
/create-use-case
UC de autenticacao com MFA: ator Usuario, fluxo principal login+OTP,
fluxo alternativo recuperacao de senha.
```

Gera um documento UC-AUTH-001 estruturado com pre-condicoes, fluxo principal,
fluxos alternativos, regras de negocio e casos de teste.

---
skill: create-use-case
category: gotcha
text: O /create-use-case esta DEPRECATED desde v3.12.0 — use /specify no lugar para criar specs de feature com user stories e criterios de aceitacao.
---
```
# DEPRECATED (remocao prevista em v4.0.0):
/create-use-case

# Use no lugar:
/specify  "feature de autenticacao com OAuth2 e MFA"
```
...
skill: specify
category: gotcha
text: O /specify cria specs — use /clarify para refinar uma spec ja existente ou /create-use-case (deprecated) para o formato antigo de UC.
---
```
# Para spec nova:
...
# Para formato UC legado (DEPRECATED):
/create-use-case  # use /specify no lugar
```
...
# ============================================================
# SECAO 3: Skills .NET (8 skills)
# ============================================================

---
skill: dotnet-create-entity
category: uso
text: Use /dotnet-create-entity para criar uma entidade de dominio .NET 10 completa com Value Objects, eventos de dominio e configuracao EF Core — seguindo DDD.
---
Para criar a entidade Order no dominio:

```
/dotnet-create-entity Order
```

Gera: classe de dominio com propriedades privadas e metodos de negocio,
Value Objects para campos com regras (ex: Money, Email), eventos de dominio,
fluent configuration EF Core e testes unitarios basicos.

---
skill: dotnet-create-entity
category: gotcha
text: O /dotnet-create-entity segue DDD — as entidades tem metodos de negocio, nao apenas getters/setters. Nao use como gerador de POCO simples.
---
Correto (DDD):

```csharp
// Gerado por /dotnet-create-entity — comportamento encapsulado:
order.Confirm();          // metodo de negocio
order.AddItem(product);   // invariante validada internamente
```

Errado (anemia de dominio — nao o que a skill gera):

```csharp
order.Status = OrderStatus.Confirmed;  // setter direto — evite
```

---
skill: dotnet-create-feature
category: uso
text: Use /dotnet-create-feature para criar uma feature .NET 10 completa com CQRS — Command, Query, Handler e Validator gerados automaticamente.
---
Para criar feature de criacao de pedido:

```
/dotnet-create-feature CreateOrder
```

Gera: CreateOrderCommand, CreateOrderCommandHandler, CreateOrderValidator
(FluentValidation), e se necessario CreateOrderQuery + Handler. Wiring no
container de DI e automatico.

---
skill: dotnet-create-feature
category: gotcha
text: O /dotnet-create-feature assume arquitetura hexagonal com CQRS — nao use em projetos com arquitetura diferente (MVC tradicional, minimal API sem mediator).
---
A skill assume:
...
Em projetos com arquitetura diferente, use /execute-task para implementacao
manual seguindo as convencoes do seu projeto.

---
skill: dotnet-create-project
category: uso
text: Use /dotnet-create-project para criar um projeto .NET 10 com arquitetura hexagonal, CQRS, Clean Code e SOLID — estrutura completa de pastas e configuracoes.
---
Para criar um novo microsservico .NET:

```
/dotnet-create-project Orders.Service
```

Gera: solucao completa com projetos Domain, Application, Infrastructure e API,
configuracao de DI, EF Core, logging estruturado e testes. Pronto para comecar
a adicionar features.

---
skill: dotnet-create-project
category: gotcha
text: O /dotnet-create-project gera a estrutura — voce ainda precisa configurar connection strings, secrets e CI/CD especificos do seu ambiente.
---
Apos /dotnet-create-project:

```bash
# 1. Configurar connection string (nao esta no codigo gerado):
dotnet user-secrets set "ConnectionStrings:Default" "..."

# 2. Configurar CI/CD no seu pipeline
# 3. Adicionar secrets ao ambiente de staging/producao
```

A skill nao cria arquivos .env ou secrets — esses ficam fora do versionamento.

---
skill: dotnet-create-test
category: uso
text: Use /dotnet-create-test para criar testes .NET 10 seguindo o padrao Triple A (Arrange, Act, Assert) com xUnit, NSubstitute e FluentAssertions.
---
Para criar testes do CreateOrderCommandHandler:

```
/dotnet-create-test CreateOrderCommandHandler
```

Gera: arquivo de teste com casos de sucesso e falha, mocks de dependencias
via NSubstitute, assertions com FluentAssertions e dados de teste via Faker.

---
skill: dotnet-create-test
category: gotcha
text: O /dotnet-create-test gera o esqueleto com casos genericos — adicione casos especificos de negocio e casos de borda manualmente.
---
Apos /dotnet-create-test, adicione:

```csharp
[Fact]
public async Task Handle_WhenOrderAmountExceedsLimit_ShouldThrowBusinessException()
{
    // Arrange: pedido com valor acima do limite
    // Act: executar handler
    // Assert: BusinessException com mensagem correta
}
```

A skill cobre o happy path — edge cases de negocio sao responsabilidade sua.

---
skill: dotnet-hexagonal-architecture
category: uso
text: Use /dotnet-hexagonal-architecture para gerar estrutura de projeto .NET 10 com arquitetura hexagonal — portas, adaptadores, domain isolado e wiring de DI.
---
Para criar estrutura hexagonal completa:

```
/dotnet-hexagonal-architecture Payments.Service
```

Gera: Domain (entidades, portas), Application (use cases, CQRS), Infrastructure
(adaptadores: banco, mensageria, HTTP), API (adaptador de entrada). Domain sem
dependencias externas.

---
skill: dotnet-hexagonal-architecture
category: gotcha
text: O /dotnet-hexagonal-architecture gera a estrutura de pastas e projetos — nao confunda com /dotnet-create-project que gera a solucao completa com configuracoes.
---
```
# Para estrutura de pastas hexagonal (layout/projetos):
/dotnet-hexagonal-architecture

# Para solucao completa com DI, EF Core, logging configurados:
/dotnet-create-project
```

As duas skills sao complementares — hexagonal-architecture define o layout,
create-project adiciona a configuracao de infraestrutura.

---
skill: dotnet-infrastructure
category: uso
text: Use /dotnet-infrastructure para configurar infraestrutura .NET 10 — PostgreSQL, RabbitMQ, gRPC e ETCD com Docker, credenciais e configuracao entre servicos.
---
Para configurar infraestrutura do servico de pagamentos:

```
/dotnet-infrastructure
PostgreSQL para persistencia, RabbitMQ para eventos de pagamento confirmado,
gRPC para comunicacao sincrona com servico de pedidos.
```

Configura: connection strings, consumers RabbitMQ, clients gRPC, ETCD para
configuracao distribuida e docker-compose para desenvolvimento local.

---
skill: dotnet-infrastructure
category: gotcha
text: O /dotnet-infrastructure configura a infraestrutura de um servico — use /dotnet-create-project primeiro para criar a estrutura base antes de adicionar infraestrutura.
---
Ordem correta:

```
# 1. Criar projeto base:
/dotnet-create-project Payments.Service

# 2. Adicionar infraestrutura:
/dotnet-infrastructure
# "PostgreSQL + RabbitMQ para Payments.Service"
```

/dotnet-infrastructure espera que a estrutura de projetos ja exista.

---
skill: dotnet-review-code
category: uso
text: Use /dotnet-review-code para revisao de codigo .NET 10 analisando Clean Code, SOLID, seguranca e performance — retorna findings priorizados.
---
Antes de fazer merge de um PR .NET:

```
/dotnet-review-code
```

Analisa o diff e retorna findings organizados por: violacoes de SOLID,
code smells, problemas de performance (N+1, chamadas sincronas em loop),
vulnerabilidades de seguranca e sugestoes de simplificacao.

---
skill: dotnet-review-code
category: gotcha
text: O /dotnet-review-code e para codigo .NET — para seguranca especifica (OWASP/ASVS) use /owasp-security em complemento, especialmente para endpoints expostos.
---
```
# Para quality/correctness em .NET:
/dotnet-review-code

# Para seguranca OWASP em endpoints expostos:
/owasp-security

# Revisao completa pre-release:
/dotnet-review-code   # depois   /owasp-security
```

---
skill: dotnet-testing
category: uso
text: Use /dotnet-testing para gerar testes unitarios e de integracao .NET 10 com xUnit, NSubstitute, FluentAssertions e padrao Triple A — cobertura completa de um modulo.
---
Para adicionar cobertura completa ao modulo de pedidos:

```
/dotnet-testing Orders
```

Gera: testes unitarios para domain e application, testes de integracao
com banco em memoria (SQLite/in-memory EF), testes de contrato para APIs.
Estrutura seguindo padrao xUnit do projeto.

---
skill: dotnet-testing
category: gotcha
text: O /dotnet-testing gera testes para um modulo inteiro — para um handler especifico use /dotnet-create-test que e mais focado e rapido.
---
```
# Para cobertura de um modulo inteiro (Orders):
/dotnet-testing Orders

# Para testes de um handler especifico:
/dotnet-create-test CreateOrderCommandHandler
```
