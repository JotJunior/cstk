...
describe('smoke: base v12+ real', () => {
  it.skipIf(!REAL || !existsSync(REAL))('abre a knowledge.db real (v12 a v14) e le as tabelas', () => {
    const r = openDb(REAL);
    expect(r.ok).toBe(true);
    if (r.ok) {
      const v = r.db.prepare("SELECT value FROM schema_meta WHERE key='schema_version'").get() as { value: string };
      // v13 (loose_usage) e v14 (plan_usage) sao aditivas — as tabelas lidas
      // abaixo existem nas tres versoes.
      expect(['12', '13', '14']).toContain(v.value);
