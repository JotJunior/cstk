...
Adapted from `JotJunior/cstk` skill `agente-00c-runtime` at commit `35922cf`.
...
Consumidos via tool Bash pelos agentes em `~/.codex/agents/agente-00c-*.md`.

## Layout

```
~/.codex/skills/cstk-agente-00c-runtime/
└── scripts/
...
```
<projeto-alvo>/.codex/agente-00c-state/
├── state.json
├── state.json.sha256
...
operador. A presenca em `~/.codex/skills/` e necessaria apenas porque o
...
`<projeto-alvo>/.codex/agente-00c-state/secrets-filter-ignore`. Razao:
