...
Adapted from `JotJunior/cstk` skill `model-selector` at commit `35922cf`.
...
quiser uma sugestao deterministica de qual modelo Codex rodar a
...
`available tools` desta skill exclui propositadamente `Task` e `Agent`
...
responsabilidade do harness Codex (dec-005). Versao concreta no
