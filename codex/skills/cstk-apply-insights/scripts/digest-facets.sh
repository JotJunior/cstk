#!/bin/sh
# digest-facets.sh — agrega os facets per-sessao do `/insights` nativo do Codex
# Code (~/.codex/usage-data/facets/*.json) num digest markdown DATA-DRIVEN,
# consumido pela skill apply-insights como fonte PRIMARIA (mais fresca e do
# projeto corrente que o resumo curado a mao em ~/.codex/insights/).
#
...
#
#   --facets-dir DIR   raiz dos *.json (default: $HOME/.codex/usage-data/facets)
...
FACETS_DIR="${HOME:-/tmp}/.codex/usage-data/facets"
SAMPLES=3
TOP=15
...
Uso: digest-facets.sh [--facets-dir DIR] [--samples N] [--top N]
  ...
  --facets-dir DIR   raiz dos facets *.json
                     (default: $HOME/.codex/usage-data/facets)
...
+ "\nFonte: ~/.codex/usage-data/facets — \($n) sessoes analisadas pelo /insights nativo.\n"
