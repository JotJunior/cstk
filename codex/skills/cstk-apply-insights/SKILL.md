...
DIGEST="$HOME/.codex/skills/cstk-apply-insights/scripts/digest-facets.sh"
...
2. Else read `~/.codex/insights/usage-insights.md` (curated; may be stale) — and
   say so to the user.
...
2. Check `.codex/settings.json` or `.codex/settings.local.json` for existing hooks
3. Check for project-level skills in `.codex/skills/`
